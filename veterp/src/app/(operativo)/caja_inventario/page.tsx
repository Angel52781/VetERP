import { getVentasResumen } from "./actions";
import { VentasList } from "./ventas-list";
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from "@/components/ui/card";
import { DollarSign, TrendingUp, AlertCircle } from "lucide-react";

export default async function CajaInventarioPage() {
  const { data: ventas, error } = await getVentasResumen();

  if (error) {
    return (
      <div className="container mx-auto py-8">
        <p className="text-red-500">Error cargando caja: {error}</p>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto py-8 space-y-8 max-w-7xl">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Gestión de ingresos, cuentas por cobrar y ventas abiertas.</p>
      </div>
